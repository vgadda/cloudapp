...
public class TopTitleStatistics extends Configured implements Tool {
    ...
    public static class TopTitlesStatMap extends Mapper<Text, Text, NullWritable, TextArrayWritable> {
        Integer N;
        TreeSet<Pair<Integer, String>> countToWords = new TreeSet<Pair<Integer,String>>();
        ...
        public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
            ...
            countToWords.add(new Pair<Integer, String>(count, word));
            
            if(countToWords.size() > N){
            	countToWords.remove(countToWords.first()); 
            }
        }

        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
        	for(Pair<Integer, String> countToWord : countToWords){
            	String[] value = {countToWord.second, countToWord.first.toString()};
            	context.write(NullWritable.get(), new TextArrayWritable(value));
            }
        }
    }
    ...
    public static class TopTitlesStatReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            ...
            sum=mean=max=var=max= new Integer(0);
            min = Integer.MAX_VALUE;

            for(TextArrayWritable value: values){
            	Text[] valueArray = (Text[])value.get();
            	countToWords.add(new Pair<Integer, String>(Integer.parseInt(valueArray[1].toString()), valueArray[0].toString()));
            ...
            for(Pair<Integer, String> countToWord : countToWords){
            	Integer count = countToWord.first;
            	sum += count;
            ...
            for(Pair<Integer, String> countToWord : countToWords){
            	Integer count = countToWord.first;
            	diffSqSum += (mean-count) * (mean-count);
            }
            var = diffSqSum/N;
